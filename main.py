...
def register():
    ...
    if form.validate_on_submit():
        ...
        if User.query.filter_by(email=email).first():
            flask.flash("Email already exists. Try to log in.")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            flask.flash("Unknown Email, Try Again.")
...
def show_post(post_id):
    ...
    if comment.validate_on_submit():
        ...
        new_comment = Comment(body=comment.body.data, author=current_user, post=post)
        db.session.add(new_comment)
        db.session.commit()
        return redirect(url_for("show_post", post_id=post_id))
    requested_post = BlogPost.query.get(post_id)
    comments_of_post = Comment.query.filter_by(post_id=post_id).all()
    return render_template("post.html", post=requested_post, form=comment, comments=comments_of_post, gravatar=gravatar)
